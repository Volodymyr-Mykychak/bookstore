...
public class ShoppingCartController implements UserContextHelper {
    ...
    @Operation(summary = "Add book to cart", description =
            "Adds a book to the cart. If the book is already there, increases " + "quantity.")
    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ShoppingCartDto addBook(@Valid @RequestBody CartItemRequestDto requestDto,
    ...
    public ShoppingCartDto updateQuantity(@PathVariable Long cartItemId,
            @Valid @RequestBody CartItemUpdateDto updateDto, Authentication authentication) {
        return shoppingCartService.updateCartItem(getUserId(authentication), cartItemId, updateDto);
    }

    @Operation(summary = "Remove a book from the cart")
    @DeleteMapping("/items/{cartItemId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteBook(@PathVariable Long cartItemId, Authentication authentication) {
